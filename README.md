...
`--host` / `--user` land the module in the `hosts/<hostname>/` / `users/<username>/` overlay instead of base (both flags together onboard into both layers; without them the module lands in `modules/`).
