...
- **Interactive commands** — hook tasks are always generated with [`interactive = true`](https://mise.jdx.dev/tasks/task-configuration.html#interactive) (issue 0088): interactivity is the hook's intrinsic property, so an interactive command in a hook (e.g. `sudo`) connects to the terminal and can disable echo on its password prompt instead of echoing it. With no terminal (piped/CI), mise runs the task as if the key were absent — hooks run normally.
