...
| `dotdrift onboard [-v|--verbose] <path>...` (alias: `add`) | Create/update module; mise apply immediately. Writes a light `module.toml` (inline `[dotfiles]` tables, no `id`/`app` — the directory name carries the identity). `-v` / `--verbose` (also `DD_VERBOSE=1`) streams the mise install/dotfiles output live, echoing each command line `set -x`-style (`+ <argv>`) to stderr before it runs |
