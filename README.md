...
| `dotdrift tui [--profile PATH]` | Open the two-pane interactive shell: module tree (Modules/Accounts/Profile) left, one active view right — resolved overlay stacks per module, raw per-layer declarations on demand (`e` opens the full-schema editor for that exact layer file: sections, validation, spliced saves that keep untouched bytes), the canonical status and plan views, accounts, and the Profile dialogs — onboard, restore with generation picking, and module generate — running through the service layer (elevated restores name their CLI command; apply arrives with T-tui-apply). |
