...
| `dotdrift status [-v\|--verbose] [-j N\|--jobs N] [--diff[=tool]] [modules...]` | Read-only drift report plus the resume cursor. Resolves the plan and probes the live system — packages (installed?), tools (`mise current`), dotfiles (symlink target / copy content / template existence), mounts (systemd unit enabled+active + destination dir), smb (group/users/services/shares) — printing `<module>: <item> — <detail>` per drifted/unknown item in fixed section order: the owning module replaces the redundant status prefix (drift is implied for listed items; unknown items get a `(?)` suffix); on a TTY, finding lines are colored by issue type (orange = missing/version drift, red = not-a-symlink/unknown, yellow = content-differs); a section whose findings are all OK prints `ok: all N checks passed`; sections with no plan items are omitted. Then a summary (`no drift` or `drift: N item(s), K unknown`), and the resume cursor line (`resume: clean …` or `resume: last completed "<step>" …`). Probes run concurrently (bounded by `-j N`; default: number of CPUs); each worker writes its own indexed result slot so the report is byte-identical regardless of worker count or completion order. `-v` / `--verbose` streams one `checking <section>: <item>` line per probe to stderr as it starts. `--diff` (bare) prints a colored unified diff for each `copy`-mode dotfile whose content differs, after the report; `--diff=tool` runs the named tool (same semantics as `apply --diff`). Exits 0 even when drift is found; only real errors (detect/load/resolve/corrupt state) return non-zero. Tools report `unknown` when mise is unavailable or the tool is not installed. |
