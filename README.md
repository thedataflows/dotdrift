...
# Onboard an existing config path into a module (--module names it)
dotdrift onboard --module bash ~/.bashrc
...
- **Interactive commands** — hook tasks are always generated with [`interactive = true`](https://mise.jdx.dev/tasks/task-configuration.html#interactive) (issue 0088) and always run on your real stdio through dotdrift's handover seam, never through a pipe dotdrift built (issue 0104): interactivity is the hook's intrinsic property, so an interactive command in a hook (e.g. `sudo`) connects to the terminal and can disable echo on its password prompt instead of echoing it. With no terminal (piped/CI), mise runs the task as if the key were absent — hooks run normally, streamed instead of captured.
...
**Onboard a live path.** Copies `~/.config/nvim` into `modules/nvim/home/.config/nvim` and links it back. `--module` is required — it names the module directory:

```bash
dotdrift onboard --module nvim ~/.config/nvim
```
...
**Profile-internal paths are rejected.** onboard/adopt brings *external* files into the profile. A path that is already inside a module layer fails loudly — declaring such a file is a hand-edit of that `module.toml` (or it rides the orphan sweep of any live-path onboard, below):

```bash
dotdrift adopt --module easyeffects ~/dotfiles/hosts/cri-pc/modules/easyeffects/home/.config/easyeffects/db/easyeffectsrc
# Error: onboard: .../easyeffectsrc is already inside module "easyeffects" (hosts/cri-pc) — onboard adopts live paths only; declare it in that module.toml directly
```

**Preview first.** `--dry-run` lists what would be onboarded and adopted, touching nothing:

```bash
dotdrift onboard --module easyeffects --dry-run ~/.config/easyeffects/db/easyeffectsrc
# would adopt: ~/.config/easyeffects/db/easyeffectsrc (home/.config/easyeffects/db/easyeffectsrc) [base]
```
...
```bash
dotdrift onboard --module easyeffects ~/.config/easyeffects/db/easyeffectsrc
```
...
| `dotdrift onboard --module <id> [--verbose] <path>...` (aliases: `add`, `adopt`) | Copy live paths into a module and apply; re-running updates the module (refresh files, merge `module.toml`). Orphaned module files (unreferenced by `[dotfiles]`) are adopted as entries with the run's mode — a fully-orphaned directory collapses to one entry (never a shared root like `~/.config`) and the live counterpart is snapshotted first; profile-internal paths are rejected with a loud error; `--dry-run` lists the would-be adoptions. |
...
| `dotdrift generate mounts&#124;smb` | Generate a mounts module (systemd units) or smb module (samba shares) into a profile layer, assembled from flags; a missing required flag errors loudly. |
| `dotdrift tui [--profile PATH]` | Open the interactive shell (M15): module nav with overlay layers left, the sectioned workspace right — `module.toml` rendered as sections with per-field inline editing over the draft ledger (conflict-refusing spliced saves), the `/` fuzzy palette (modules, valid actions, fields), `p` plan and `P` apply with the destructive confirm + one elevation modal, the writes dialogs (onboard/restore/generate) on `w`, module management on `m`. One keymap as data; `?` shows the contextual table. |
