...
```bash
dotdrift onboard --app easyeffects --dry-run ~/.config/easyeffects/db/easyeffectsrc
...
| `dotdrift onboard --app <id> [--verbose] <path>...` (aliases: `add`, `adopt`) | Copy live paths into a module and apply; re-running updates the module (refresh files, merge `module.toml`). Orphaned module files (unreferenced by `[dotfiles]`) are adopted as entries with the run's mode — a fully-orphaned directory collapses to one entry (never a shared root like `~/.config`) and the live counterpart is snapshotted first; profile-internal paths are rejected with a loud error; `--dry-run` lists the would-be adoptions. |
...
| `dotdrift tui [--profile PATH]` | Open the two-pane interactive shell: module tree (Modules/Accounts/Profile) left, one active view right — resolved overlay stacks per module, raw per-layer declarations on demand (`e` opens the full-schema editor for that exact layer file: sections, validation, spliced saves that keep untouched bytes), the canonical status and plan views, accounts, the Profile dialogs (onboard, restore with generation picking, module generate), and streamed apply: `a` on the plan view opens the gate (per-step TTY reasons), the session streams into a progress view-model, sudo/interactive steps take the real terminal, cancel is gated (T-tui-apply). The M15 compositor shell (issue 0073, in progress) is reachable behind `dotdrift tui --compositor`: one shell plus a modal stack, inline editing, `/` fuzzy palette, and a single keymap — `?` shows the contextual table. |
