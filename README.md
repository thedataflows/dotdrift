...
`--host=<hostname>` / `--user=<username>` land the module in that host's/user's layer instead of base (both flags together onboard into both layers; without them the module lands in `modules/`).
