...
// stubKernelRelease comes from registry_test.go (same package).
...
func TestWizard_kernelRecommendedTypeIsDefault(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.kernel, func(t *testing.T) {
			...
			vols := []Volume{{UUID: "aaaa-bbbb", FSType: "ntfs", Label: "data"}}
...
func TestWizard_addMountValidation(t *testing.T) {
	...
	bad := NewMountsWizard(Selection{Layer: LayerBase, ModuleID: "mounts"}, reg)
...
func TestSmbWizard_writeRequiresShare(t *testing.T) {
	w := NewSmbWizard(Selection{Layer: LayerBase, ModuleID: "smb"})
