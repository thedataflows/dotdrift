...
| `dotdrift status [-v\|--verbose] [-j N\|--jobs N] [--diff[=tool]] [modules...]` | Read-only drift report plus the resume cursor. Resolves the plan and probes the live system — packages (installed?), tools (`mise current`), dotfiles (whole-file: symlink target / copy content / template existence; edit entries: exact line match, marker-delimited block content, or marker presence for template), mounts (systemd unit enabled+active + destination dir), smb (group/users/services/shares) — printing `<module>: <item> - <detail>` per drifted/unknown item in fixed section order, packages grouped by owning module then sorted by package name: the owning module replaces the redundant status prefix (drift is implied for listed items; unknown items get a `(?)` suffix); on a TTY, finding lines are colored by issue type (orange = missing/version drift, red = not-a-symlink/unknown, yellow = content-differs) with the module in a faint shade and the item in a bold shade of that hue; a section whose findings are all OK prints `ok: all N checks passed`, the summary is `no drift` or `drift: N item(s)` (`, K unknown` appended). Symlink sources are validated: a link pointing at the right path whose source file was deleted reports `source missing (dangling link)`, and a symlink-each target-dir link into the entry's source dir whose source was removed reports `stale link: source removed`. A trailing `orphans:` section lists files inside selected modules' layer directories that no `[dotfiles]` entry references — neither as a source nor as part of a `symlink-each` source subtree (the whole tree under a symlink-each source in the declaring layer's module.toml counts as referenced) — grouped under layer-root headings (`base:`, `hosts/<hostname>:`, `users/<username>:`), one `<module>: <file> - not referenced by [dotfiles]` line per orphan; on a TTY orphan lines render in a distinct magenta shade, apart from the live-system drift hues. The section is omitted when empty. `--diff` shows colored unified diffs for copy-mode dotfiles whose content differs. `-v` streams one `checking <section>: <item>` line per probe to stderr; `-j N` bounds concurrent probe workers (0 = NumCPU). Exits 0 even when drift is found |
